...
variable "db_redis_bus_port" {
  description = "The port Redis (or Valkey) Sharded Cluster management port"
  type        = string
  default     = "16379"
}

variable "db_redis_node_timeout_ms" {
  description = "Redis (and Valkey) Sharded Cluster node timeout in milliseconds"
  type        = number
  default     = 15000
}

variable "db_redis_replica_validity_factor" {
...
variable "host_role" {
  description = "The role, either Primary or Replica, of the host in a Redis or Valkey Sharded Cluster"
  type        = string
  default     = "primary"
}
